...
class TestApp(unittest.TestCase):
    def setUp(self):
        ...
        self.bill = app.to_json(self.bill)

        self.debt = debts.Debt('Debt Name', 123, 4, 10)
        self.debt = app.to_json(self.debt)
    ...
    def test_duplicate_name(self):
        ...
        answer = app.duplicate_name(unexpected, self.session['bills'])
        answer1 = app.duplicate_name(expected_bill, self.session['bills'])
        answer2 = app.duplicate_name(expected_debt, self.session['debts'])
    ...
    def test_session_append(self):
        json_apendee_bill = app.to_json(self.apendee_bill)
        json_apendee_debt = app.to_json(self.apendee_debt)

        app.session_append(self.session['bills'], json_apendee_bill)
        app.session_append(self.session['debts'], json_apendee_debt)

        self.assertIn(json_apendee_bill, self.session['bills'])
        self.assertIn(json_apendee_debt, self.session['debts'])

    def test_strip_whitespace(self):
        bill_answer = app.strip_whitespace(self.white_space_bill.name)
        debt_answer = app.strip_whitespace(self.white_space_debt.name)
